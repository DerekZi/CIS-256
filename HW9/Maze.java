  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    try{
      ...
      for (int k=0;k<numOfWalls;k++){
        counter = walls[k];
        if (counter>0){
          int num1=set.find(counter);
          int num2=set.find(counter + horiz);
          if (num1!=num2){
            set.union(num1,num2);
            hWalls[counter%horiz][counter/horiz]=false;
          }
        }else {
          int num1=set.find(-counter);
          int num2=set.find(-counter + 1);
          if (num1!=num2){
            set.union(num1,num2);
            vWalls[(-counter)%horiz][(-counter)/horiz]=false;
          }
        }
      }
    }catch(Exception e){
