  ...
  public UDGraph length2Paths() {
    ...
    for(int v=0;v<vertices;v++){
      for(int u=0;u<vertices;u++){
        if(hasEdge(v, u)){ // check whether there is an edge (v,u)
          for(int w=0;w<vertices;w++){
            if(hasEdge(u, w)){ //if there is an edge (v,u), then check whether there is an edge (u,w).
              newGraph.addEdge(v, w); //if there are edges (v,u) and (u,w), then add edge (v,w) to the newGraph since the path goes from v to w has a length of 2
            }
          }
        }
      }
