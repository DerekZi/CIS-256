...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    if(isValidNode()){
      ...
      insert.parent = this;
      SibTreeNode ptr = firstChild;
      if(c <= 1||firstChild==null){  
        firstChild = insert;
        insert.nextSibling = ptr;
      }else{
        SibTreeNode cur = firstChild.nextSibling;
        while((cur!=null)&&c>2){
          ptr = ptr.nextSibling;
          cur = cur.nextSibling;
          c--;
        }
        ptr.nextSibling = insert;
        insert.nextSibling = cur;
      }
      myTree.size++;
    }else{
      throw new InvalidNodeException();
  ...
  public void removeLeaf() throws InvalidNodeException {
    if(isValidNode()){
      if(children()==0){
        ...
        else if(this==parent.firstChild){
