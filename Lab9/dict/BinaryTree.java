...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    BinaryTreeNode found = findHelper((Comparable)key, root);
    if(found==null){
      return null;
    }else{
      //no child
      ...
      //two children
        ...
        BinaryTreeNode rightSub = found.rightChild;
        while(rightSub.leftChild!=null){
          rightSub = rightSub.leftChild;
        }
        if(found.parent==null){
          root=rightSub;
        }
        if(found==found.parent.leftChild){
          found.parent.leftChild.entry.key=rightSub.entry.key();
          found.parent.leftChild.entry.value=rightSub.entry.value();
        }else if(found==found.parent.rightChild){
          found.parent.rightChild.entry.key=rightSub.entry.key();
          found.parent.rightChild.entry.value=rightSub.entry.value();
        }
        if(rightSub.rightChild==null){
          if(rightSub==rightSub.parent.leftChild){
            rightSub.parent.leftChild=null;
          }else{
            rightSub.parent.rightChild=null;
          }
        }else{
          if(rightSub==rightSub.parent.leftChild){
            rightSub.parent.leftChild=rightSub.rightChild;
          }else{
            rightSub.parent.rightChild=rightSub.rightChild;
          }
